"""Deploy the **research MCP server** as an Azure Container App.

The NorthStar Health innovation / research knowledge base (Azure AI Search
backed) that the strategy agent reaches through the ``strategy_toolbox``. Run it
after ``azd up`` and after the ``innovations`` search index has been created and
ingested.

Usage::

    python -m scripts.deploy_research_mcp_server --build           # build + deploy
    python -m scripts.deploy_research_mcp_server                   # deploy only
...
Environment variables (populated automatically from ``.env`` after ``azd up``):
  ...
  RESEARCH_MCP_EXTERNAL                  "true" for public ingress (default: true)
...
IMAGE_NAME = "research-mcp-server"
PORT = int(os.getenv("RESEARCH_MCP_PORT", "8095"))
_DOCKERFILE = "src/research_mcp_server/Dockerfile"
...
def deploy(tag: str | None = None) -> str:
    ...
    if fqdn:
        print(f"\nResearch MCP server deployed: https://{fqdn}/mcp")
    else:
