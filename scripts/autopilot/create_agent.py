...
Python port of ``agent-creation-script.ps1``. It:
  ...
  1. POSTs a new hosted agent version that references the ACR image and the
     Managed Agent Identity Blueprint, using the ``activity_protocol``. The
     NorthStar Health marketing toolbox endpoint is injected as a container
     environment variable so campaign plans and reports are grounded in data.
...
def create_agent(config: AutopilotConfig | None = None) -> str:
    ...
    body = {
        ...
        "description": "NorthStar Health campaign-driving A365 digital worker.",
