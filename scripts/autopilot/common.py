"""Shared helpers for the NorthStar Health campaign A365 autopilot scripts.
...
class AutopilotConfig:
    ...
    @property
    def marketing_toolbox_endpoint(self) -> str:
        """The marketing_toolbox MCP consumer endpoint for this project."""
        explicit = os.getenv("MARKETING_TOOLBOX_MCP_ENDPOINT", "").strip()
        if explicit:
            return explicit
        return (
            f"{self.project_endpoint}/toolboxes/{self.marketing_toolbox_name}"
            "/mcp?api-version=v1"
        )
    ...
    def from_env(cls) -> "AutopilotConfig":
        ...
        marketing_toolbox_name = os.getenv(
            "MARKETING_TOOLBOX_NAME", "marketing_toolbox"
        ).strip()
        ...
        return json.dumps(
            {
                ...
                "marketing_toolbox_name": self.marketing_toolbox_name,
                "marketing_toolbox_endpoint": self.marketing_toolbox_endpoint,
            },
            indent=2,
        )
