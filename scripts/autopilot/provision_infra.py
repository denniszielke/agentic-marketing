...
def provision_infra(config: AutopilotConfig | None = None) -> str:
    ...
    # Azure Bot handles are GLOBALLY unique. The bicep default appends a
    # per-resource-group/account hash to avoid collisions; set CAMPAIGN_A365_BOT_NAME
    # to pin an explicit (still globally unique) name.
    bot_name = os.getenv("CAMPAIGN_A365_BOT_NAME", "").strip()
    if bot_name:
        params.append(f"botName={bot_name}")
