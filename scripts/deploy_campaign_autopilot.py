"""Deploy the NorthStar Health campaign A365 *autopilot* digital worker — end to end.
...
outputs, deploy the marketing MCP servers and register ``marketing_toolbox``
(so the digital worker can ground campaign plans and reports in NorthStar Health
data), and be logged in with ``az login`` (Owner on the subscription).
...
def deploy(args: argparse.Namespace) -> int:
    ...
    print("\n✅ NorthStar Health campaign A365 autopilot deployment finished.")
    print(f"   AGENT_IDENTITY_BLUEPRINT_ID = {blueprint_id}")
    print(f"   AGENT_NAME                  = {config.agent_name}")
    print(f"   MARKETING_TOOLBOX_ENDPOINT  = {config.marketing_toolbox_endpoint}")
    print(
