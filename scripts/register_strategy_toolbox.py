"""Register the **strategy_toolbox** Foundry toolbox.

Creates (or updates) the Foundry toolbox that the ``executive_strategy_agent``
consumes at runtime. It bundles four remote MCP server Container Apps — the three
in the marketing toolbox plus the research/innovation knowledge base:
  ...
  * ``research-mcp-server``     (innovation / research knowledge base)

Run this after the four MCP server Container Apps are deployed.

Auth: honours ``ENTRA_AUTH_ENABLED``. When on, each server is registered with a
``project_connection_id`` (agent-identity connection); when off, the plain server
URL is registered.
...
Environment variables:
  ...
  *_MCP_URL / *_MCP_CONNECTION_ID   explicit URLs / connection ids (optional).
"""
...
TOOLBOX_NAME = os.getenv("STRATEGY_TOOLBOX_NAME", "strategy_toolbox")
...
_SERVERS = [
    ...
    ("research-data", "RESEARCH_MCP_URL", "RESEARCH_MCP_APP_NAME", "research-mcp-server",
     "RESEARCH_MCP_CONNECTION_ID",
     "NorthStar Health innovation and research knowledge base."),
]
...
def deploy() -> None:
    ...
    version = client.toolboxes.create_version(
        name=TOOLBOX_NAME,
        tools=tools,
        description="Strategy toolbox: product, market-insights, persona and research MCP servers.",
