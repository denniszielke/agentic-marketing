"""Deploy the **product MCP server** as an Azure Container App.

The NorthStar Health product catalogue (Azure AI Search backed) that the
marketing and strategy agents reach through the ``marketing_toolbox`` /
``strategy_toolbox``. Run it after ``azd up`` has provisioned the infrastructure
and the ``products`` search index has been created and ingested.

Usage::

    python -m scripts.deploy_product_mcp_server --build            # build + deploy
    python -m scripts.deploy_product_mcp_server                    # deploy only
...
Environment variables (populated automatically from ``.env`` after ``azd up``):
  ...
  PRODUCT_MCP_EXTERNAL                   "true" for public ingress (default: true)
...
APP_NAME = os.getenv("PRODUCT_MCP_APP_NAME", "product-mcp-server")
IMAGE_NAME = "product-mcp-server"
PORT = int(os.getenv("PRODUCT_MCP_PORT", "8093"))
_DOCKERFILE = "src/product_mcp_server/Dockerfile"
...
def deploy(tag: str | None = None) -> str:
    ...
    if fqdn:
        print(f"\nProduct MCP server deployed: https://{fqdn}/mcp")
    else:
        print("\nProduct MCP server deployed, but no ingress FQDN was returned.")
    return fqdn
