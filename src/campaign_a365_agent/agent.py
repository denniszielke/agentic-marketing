...
"""CampaignDigitalWorker — NorthStar Health campaign-driving A365 digital worker.
...
``agent_framework`` dependency) and passes two MCP server bundles on every turn:

* **Microsoft 365 delivery tools** from :file:`ToolingManifest.json` (Word,
  Excel, PowerPoint, Mail, Teams, OneDrive/SharePoint, Calendar) so the digital
  worker can deliver campaign plans and reports where the user works.
* **NorthStar Health marketing tools** — the ``marketing_toolbox`` Foundry
  toolbox (product catalogue + market insights + personas), injected at runtime
  from ``MARKETING_TOOLBOX_MCP_ENDPOINT`` / ``MARKETING_MCP_URL`` — so campaign
  recommendations and reports are grounded in real product, sales, margin,
  growth, market-share, forecast and persona data.
...
# Audience used to acquire the agentic-user token that the M365 MCP servers
# accept. Matches the C# ResponsesApiAgentLogicServiceFactory.
...
# Default scope used to acquire a token for the NorthStar Health marketing
# toolbox (the Foundry data plane). Override with MARKETING_MCP_SCOPE.
MARKETING_SCOPE_DEFAULT = "https://ai.azure.com/.default"
...
class CampaignDigitalWorkerAgent(AgentInterface):
    """NorthStar Health campaign-driving A365 digital worker (calls AOAI directly)."""

    AGENT_PROMPT = (
        "You are the NorthStar Campaign Planner, a marketing digital worker for "
        "NorthStar Health — a consumer-healthcare company selling Vitamins & "
        "Supplements, Gut Health, Weight Management and Home Diagnostics across "
        "Germany, the UK and the Nordics. You drive marketing campaigns and "
        "report on their performance so the business grows profitably against "
        "competitors (NordicVital, PurePath Wellness, VitaCore Labs, NutriSphere, "
        "BioMetric Health) WITHOUT eroding margin.\n\n"
        ...
        "- Campaign driving — design a margin-aware campaign for a market, "
        "category and persona: find the competitive and white-space gaps, pick "
        "3-6 hero products, set the promotional depth at the deepest cut that "
        "still keeps campaign gross margin flat-to-accretive, choose the persona-"
        "matched claims, channels and messaging, and forecast the sales and "
        "margin impact.\n"
        "- Campaign reporting — pull sales, market share, growth, margin and "
        "forecast for the relevant market/category and build a decision-ready "
        "performance report: what moved, why, category contribution, competitive "
        "position and next actions.\n"
        "- Portfolio analysis — rank categories and products by margin "
        "contribution and growth, overlay personas and competitive pressure, and "
        "classify each as Defend / Grow / Traffic / Fix.\n"
        "- Persona targeting — tailor claims, channel mix and messaging to the "
        "target persona's motivations, barriers and preferred channels.\n\n"
        "# Grounding — always use the marketing tools\n"
        "Ground every recommendation and report in NorthStar Health data via the "
        "marketing MCP tools, never on assumptions:\n"
        "- Product catalogue tools — NorthStar and competitor products, claims, "
        "price tiers, markets and margins.\n"
        "- Market-insights tools — sales, market share, growth rates, margin "
        "comparison and forecast by market and category.\n"
        "- Persona tools — customer segmentation, motivations and channels.\n\n"
        "# Operating principles\n"
        "1. Optimise for PROFITABLE GROWTH — weigh revenue and volume uplift "
        "against gross margin; never chase market share at the expense of "
        "margin, and never reason on headline price alone.\n"
        "2. Never propose a promotional price at or below the product's cost "
        "floor.\n"
        "3. Treat unit cost and procurement data as confidential: reason with "
        "it, but report only prices, margin percentages and margin deltas — "
        "never raw cost.\n"
        "4. Health-product claims must stay compliant — only use the claims that "
        "the product catalogue lists for a product; never invent medical "
        "claims.\n"
        "5. Justify every recommendation with the numbers behind it and flag the "
        "key risks (competitor counter-move, claims/regulatory compliance, "
        "cannibalisation, cost-floor proximity).\n\n"
        "# Microsoft 365 delivery\n"
        "You are a hireable digital worker with Microsoft 365 tools. Use them to "
        "deliver the campaign or report where the user works:\n"
        "- Use the Excel MCP tools to build campaign plans and performance "
        "reporting workbooks (sales, margin, share, forecast).\n"
        "- Use the Word MCP tools to draft the campaign brief or the written "
        "performance report.\n"
        "- Use the PowerPoint MCP tools to build a campaign or results deck when "
        "the user asks for slides.\n"
        "- Use the SendEmail function to send responses back when handling "
        "email; use professional, formal language. You can use the AAD object ID "
        "inside the Activity context's 'From' field to determine where to "
        "reply.\n"
        "- For Teams messages, only use the Teams MCP tool when a user "
        "explicitly asks to send a Teams message. Otherwise, do not use it.\n\n"
        "# General\n"
    ...
    def __init__(self) -> None:
        ...
        # Microsoft 365 delivery MCP servers (Word/Excel/PowerPoint/Mail/Teams/...).
        self._mcp_servers = self._load_mcp_servers()
        self._mcp_token_override = os.getenv("BEARER_TOKEN") or None

        # NorthStar Health marketing MCP servers (marketing_toolbox).
        self._marketing_servers = self._load_marketing_servers()
        self._marketing_token_override = (
            os.getenv("MARKETING_TOOLBOX_BEARER_TOKEN") or None
        )
        self._marketing_scope = os.getenv("MARKETING_MCP_SCOPE") or None
        ...
        logger.info(
            "✅ Foundry agent ready (endpoint=%s, deployment=%s, m365_servers=%d, "
            "marketing_servers=%d)",
            self._endpoint,
            self._deployment,
            len(self._mcp_servers),
            len(self._marketing_servers),
        )
    ...
    def _load_mcp_servers(self) -> list[dict[str, Any]]:
        ...
        logger.info("Loaded %d M365 MCP server(s) from ToolingManifest.json", len(servers))
        return servers

    def _load_marketing_servers(self) -> list[dict[str, Any]]:
        """Resolve the NorthStar Health marketing_toolbox MCP endpoint (if any).

        Prefers an explicit ``MARKETING_TOOLBOX_MCP_ENDPOINT`` / ``MARKETING_MCP_URL``.
        Otherwise, if ``AZURE_AI_PROJECT_ENDPOINT`` is set, derives the Foundry
        toolbox consumer endpoint ``{project}/toolboxes/{toolbox}/mcp?api-version=v1``.
        """
        endpoint = (
            os.getenv("MARKETING_TOOLBOX_MCP_ENDPOINT", "").strip()
            or os.getenv("MARKETING_MCP_URL", "").strip()
        )
        if not endpoint:
            project = os.getenv("AZURE_AI_PROJECT_ENDPOINT", "").strip()
            toolbox = os.getenv("MARKETING_TOOLBOX_NAME", "marketing_toolbox").strip()
            if project:
                endpoint = (
                    f"{project.rstrip('/')}/toolboxes/{toolbox}/mcp?api-version=v1"
                )
        if not endpoint:
            logger.info(
                "No marketing toolbox endpoint configured (set "
                "MARKETING_TOOLBOX_MCP_ENDPOINT, MARKETING_MCP_URL, or "
                "AZURE_AI_PROJECT_ENDPOINT). Campaign grounding tools disabled."
            )
            return []

        label = os.getenv("MARKETING_TOOLBOX_SERVER_LABEL", "marketing_toolbox").strip()
        logger.info("Loaded marketing toolbox MCP endpoint: %s", endpoint)
        return [
            {
                "mcpServerName": label,
                "url": endpoint,
                "description": (
                    "NorthStar Health marketing data: product catalogue (internal "
                    "and competitor products, claims, price tiers, margins), market "
                    "insights (sales, market share, growth, margin comparison, "
                    "forecast) and customer personas."
                ),
            }
        ]
...
""".strip()
    ...
    async def _build_mcp_tools(
        ...
        if not self._mcp_servers and not self._marketing_servers:
            return []
        ...
        # 1. Microsoft 365 delivery servers — authenticated with the agentic
        #    user token (MCP_SCOPE).
        for server in self._mcp_servers:
            tool = self._make_mcp_tool(server, bearer)
            if tool:
                tools.append(tool)

        # 2. NorthStar Health marketing servers — authenticated with the
        #    marketing token (override / MARKETING_MCP_SCOPE / agentic fallback).
        if self._marketing_servers:
            marketing_bearer = await self._acquire_marketing_token(bearer)
            for server in self._marketing_servers:
                tool = self._make_mcp_tool(server, marketing_bearer)
                if tool:
                    tools.append(tool)
    ...
    @staticmethod
    def _make_mcp_tool(
        server: dict[str, Any], bearer: Optional[str]
        ...
        tool: dict[str, Any] = {
            ...
            "server_label": name,
            "server_url": url,
            "server_description": server.get("description") or f"MCP server: {name}",
    ...
    async def _acquire_marketing_token(
        self, fallback_bearer: Optional[str]
    ) -> Optional[str]:
        """Bearer token attached to the marketing_toolbox MCP requests.

        Preference order: explicit override → token for MARKETING_MCP_SCOPE via
        the agent credential → the shared agentic MCP bearer (works when the
        marketing toolbox is exposed through the same A365 agentic auth).
        """
        if self._marketing_token_override:
            return self._marketing_token_override

        if self._aoai_credential is not None:
            scope = self._marketing_scope or MARKETING_SCOPE_DEFAULT
            try:
                token = await self._aoai_credential.get_token(scope)
                return token.token
            except Exception:
                logger.exception(
                    "Failed to acquire marketing toolbox token for scope %s", scope
                )
